...
| D6 | **RESOLVED (W2.3).** Was: Buddy profile save mutated in-memory `Rx<UserModel>` only. Now persists via `ProfileRepository.updateBuddyProfile`; state updates only after backend confirmation, and schema-missing failures are reported honestly (column contract D-45/C-AUTH-03 still OPEN). | `controllers/profile_controller.dart › saveBuddyProfile`; `lib/data/profile_repository_supabase.dart` |
| D7 | **RESOLVED as a fake-success (W2.3).** Was: document "upload" wrote the sentinel `'pending-upload'` with no picker/Storage call. `uploadDocument` now reports the missing storage contract (C-DOC-01..05/D-47) honestly — real upload remains blocked until the backend answers the contract. | `controllers/profile_controller.dart › uploadDocument` |
...
| FR-PROF-02 | Buddy can edit own profile at any time, except email | `profile_screen.dart › _openEditSheet` › `ProfileController.saveBuddyProfile` | UI ✅ · Logic ✅ · Data ◐ | **U** | Since W2.3 the save persists via `ProfileRepository` (email untouched, sheet stays open on failure); column names beyond `full_name` remain assumed pending D-45/C-AUTH-03 | Answer the user-field contract (D-45); keep email read-only | FR-PROF-01 | W2 / P1 | — |
...
| FR-PROF-04 | Tutor profile completion form: bio, subjects taught, grade levels taught | `views/tutor/tutor_profile_screen.dart` (bio, subjects, extraSkills) › `saveTutorProfile` | UI ◐ · Logic ◐ · Data ◐ | **P** | Since W2.3 bio/subjects/extraSkills persist via `ProfileRepository` (profile sourced from the backend, no dummy); "jenjang yang bisa diajar" is still absent and D-23's data-shape question stands | Add grade-levels-taught to the Tutor profile and persist the form | FR-DISC-03 | W2 / P1 | Data shape for "jenjang yang bisa diajar" (not in SRS §4.1 entity attributes) |
| FR-PROF-05 | Upload Tutor documents: (a) transcript, (b) student ID (KTM/KTS), (c) achievement certificate, (d) language certificate (only for foreign-language tutors) | `domain/profile_repository.dart › SrsDocumentType` (SRS set); `views/shared/widgets/document_tile.dart` | UI ◐ · Logic ◐ · Data ✗ | **M** ⚠ contradicts SRS | Since W2.3 the conflicting six-document catalogue is no longer used as a data source: the four SRS-named types carry their SRS classification, other types are reported unclassified, and documents are read from the backend via `ProfileRepository`. Upload itself remains unimplemented (D7) pending the storage contract (D-47) | Answer the storage contract (D-47/C-DOC-01..05), then implement selection + upload | NFR-PROF-01, NFR-PROF-02, NFR-PROF-03 | W2 / P1 | Whether UTBK/CV documents are ratified additions |
...
| FR-PROF-09 | Tutor self-service editing incl. bio, subjects, and adding/replacing certificates after initial verification | `tutor_profile_screen.dart` (edit sheet, per-document `Upload`/`Ganti`) › `ProfileController.saveTutorProfile`/`uploadDocument` | UI ✅ · Logic ◐ · Data ◐ | **U** | Since W2.3 bio/subjects/extraSkills persist via `ProfileRepository`; the fake post-verification reset to `menunggu` was removed, and document upload reports the missing storage contract instead (D7). Re-verification semantics remain open (D-22) | Define what re-verification means when a certificate is added/replaced after approval | FR-PROF-05, FR-PROF-07 | W2 / P1 | Whether adding a certificate re-triggers verification (not addressed by SRS) |
...
| "Umum" grade option; UTBK score & CV documents | `profile_screen.dart`; former `_dummyDocuments` catalogue (removed as a data source in W2.3) | Contradict the SRS option/document sets (see FR-AUTH-06, FR-PROF-05) |
